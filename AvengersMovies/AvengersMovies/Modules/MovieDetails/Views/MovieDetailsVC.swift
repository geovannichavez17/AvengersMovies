...
class MovieDetailsVC: BaseVC {
    ...
    lazy var trailerNotAvailable: UILabel = {
        let label = UILabel()
        label.text = "No trailers availables for this film"
        ...
        viewModel?.videoItems.bind { [weak self] videoThumbnail in
            ...
            DispatchQueue.main.async {
                if videoThumbnail.isEmpty {
                    self.content.addSubview(self.trailerNotAvailable)
                    self.trailerNotAvailable.centerXAnchor.constraint(equalTo: self.trailerGuide.centerXAnchor).isActive = true
                    self.trailerNotAvailable.centerYAnchor.constraint(equalTo: self.trailerGuide.centerYAnchor).isActive = true
                    self.trailerNotAvailable.widthAnchor.constraint(equalTo: self.trailerGuide.widthAnchor, multiplier: 0.50).isActive = true
                } else {
                    if let trailerLabel = self.content.viewWithTag(20) {
                        trailerLabel.removeFromSuperview()
                    }
